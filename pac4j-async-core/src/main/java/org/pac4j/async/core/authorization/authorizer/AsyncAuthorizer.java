...
public interface AsyncAuthorizer<U extends CommonProfile> {
     ...
     * one
     */
    static <U extends CommonProfile> AsyncAuthorizer<U> fromNonBlockingAuthorizer(final Authorizer<WebContext, U> syncAuthorizer) {
        return (context, profiles) -> AsynchronousComputationAdapter.fromNonBlocking(ExceptionSoftener.softenSupplier(() -> syncAuthorizer.isAuthorized(context, profiles)));
    }
